...
import { Zap, Grid, Fingerprint, FileText, ScanLine, Activity } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ currentSection, onNavigate }) => {
  ...
  return (
    <nav className="h-20 border-b border-white/10 bg-[#020617]/80 backdrop-blur-md sticky top-0 z-50 flex items-center justify-between px-6 md:px-12">
      ...
      {/* Navigation Tabs */}
      <div className="hidden md:flex items-center gap-2 bg-[#0f172a] p-1.5 rounded-full border border-white/5">
        {navItems.map((item) => {
